...
impl Histogram {
  ...
  fn thresholding_build_lookup_table(&mut self) -> Vec<u32> {
    let mut P: Vec<u32> = Vec::with_capacity(COLOR_DEPTH * COLOR_DEPTH);
    let mut S: Vec<u32> = Vec::with_capacity(COLOR_DEPTH * COLOR_DEPTH);
    let mut H: Vec<u32> = Vec::with_capacity(COLOR_DEPTH * COLOR_DEPTH);
    let pixels_total = self.pixels;

    // diagonal
    for col in (1..COLOR_DEPTH).skip(1) {
      let idx = index(col, col);
      let tmp = (self.lum[col] as usize / pixels_total) as u32;
      P[idx] = tmp;
      S[idx] = col as u32 * tmp;
    }
  ...
  pub fn multilevel_thresholding(
    ...
    fn iterate_recursive(
      ...
      for i in start..(max - amount + prev_depth) {
        ...
        if depth + 1 < amount + 1 {
          return iterate_recursive(amount, max, sig, h, color_stops, i, variance, ids, depth);
        } else {
  ...
  pub fn get_dominant_color(
    ...
    for i in min..(max + 1) {
      ...
      for j in ((tol as i8 / -2) as u8)..tol {
        tmp += if between((i + j) as f64, 0.0, COLOR_RANGE_END as f64) {
          colors[(i + j) as usize]
        } else {
          0
        }
      ...
      let sum_equal_but_main_color_is_bigger =
        dom_val == tmp as isize && (dom_idx < 0 || colors[i as usize] > colors[dom_idx as usize]);
  ...
  pub fn get_stats(
    ...
    level_min: f64,
    level_max: f64,
    refresh: Option<Box<bool>>, // = false
  ) -> Stats {
    let [min, max] = normalize_min_max(Some(Box::new(level_min)), Some(Box::new(level_max)));
    let cache_key = format!("{}-{}", min, max).to_string();
    let rf = match refresh {
      Some(val) => *val,
      None => false,
    };

    if !rf && self.cached_stats.contains_key(&cache_key) {
    ...
    let mut all_pixel_values_combined = 0;
    let mut unique_values = 0; // counter for levels that's represented by at least one pixel
    let mut most_pixels_per_level = 0;

    // Finding number of pixels and mean
    for i in min..(max + 1) {
      pixels_total += data[i as usize] as i32;
      all_pixel_values_combined += data[i as usize] * i;
      unique_values += if data[i as usize] == 0 { 0 } else { 1 };
      if most_pixels_per_level < data[i as usize] as i32 {
        most_pixels_per_level = data[i as usize] as i32
      }
    }

    let mean_value = (all_pixel_values_combined as i32 / pixels_total) as u8;
    let median_pixel_index = (pixels_total as f64 / 2.0).floor();
    let mut tmp_pixels_iterated = 0;
    let mut tmp_sum_of_deviations = 0;

    // Finding median and standard deviation
    for i in 0..COLOR_DEPTH {
      let tmp_pixel_value = sorted_indexes[i];
      let tmp_pixels = data[tmp_pixel_value];
      // eslint-disable-next-line
      if tmp_pixel_value < min as usize || tmp_pixel_value > max as usize {
        continue;
      }
      tmp_pixels_iterated += tmp_pixels;
      tmp_sum_of_deviations += (tmp_pixel_value - mean_value as usize).pow(2) * tmp_pixels as usize;
      if median_value.is_none() && tmp_pixels_iterated >= median_pixel_index as u8 {
        median_value = Some(tmp_pixel_value as u8)
      }
    }
    ...
    self.cached_stats.insert(
      ...
      Stats {
        ...
        levels: Levels {
          mean: mean_value,
          median: median_value,
          std_dev: (tmp_sum_of_deviations as f64 / pixels_total as f64).sqrt(),
          unique: unique_values,
        },
        // what's visually represented as bars
        pixels_per_level: PixelsPerLevel {
          mean: pixels_total / (max - min) as i32,
          median: pixels_total / unique_values,
